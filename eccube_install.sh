...
"mysql" ) 
    # MySQL
    echo "dropdb..."
    ${MYSQL} -u ${ROOTUSER} -e "drop database ${DBNAME}"
    echo "createdb..."
    ${MYSQL} -u ${ROOTUSER} -e "create database ${DBNAME}"
    #echo "grant user..."
    #${MYSQL} -u ${ROOTUSER} -p${ROOTPASS} -e "GRANT ALL ON ${DBNAME}.* TO '${DBUSER}'@'%' IDENTIFIED BY '${DBPASS}'"
    echo "create table..."
    ${MYSQL} -u ${DBUSER} ${DBNAME} < ${SQL_DIR}/create_table_mysql.sql
    echo "insert data..."
    ${MYSQL} -u ${DBUSER} ${DBNAME} < ${SQL_DIR}/insert_data.sql
    for S in $SEQUENCES
    do
    ...
    echo "execute optional SQL..."
    ${MYSQL} -u ${DBUSER} ${DBNAME} < ${OPTIONAL_SQL_FILE}
;;
esac
